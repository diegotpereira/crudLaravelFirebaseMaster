...
<head>
    ...
    <title>Laravel Crud Firebase</title>
</head>
<body>
<div class="container" style="margin-top: 50px;">
    <h4 class="text-center">Laravel Realtime</h4><br>
    <h5>#Novo Cliente</h5>
    <div class="card card-default">
        <div class="card-body">
            <form id="addCliente" class="form-inline" method="POST" acation="">
                ...
                <button id="submitCliente" type="button" class="btn btn-primary mb-2">Salvar</button>
            </form>
        </div>
    </div>
    <br>
...
<form action="" method="POST" class="user-update-record-model form horizontal">
        ...
        <div class="modal-dialog modal-dialog-centered" style="width: 55%;">
            <div class="modal-content" style="overflow: hidden;">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-success updateCliente">Atualizar</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="remove-modal" data-backdrop="static" data-keyboard="false" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="custom-width-modalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog modal-dialog-centered" style="width:55%;">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="custom-width-modalLabel">Excluir</h4>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default waves-effect remove-data-from-delete-form" data-dismiss="modal">Sair</button>
                    <button type="button" class="btn btn-danger waves-effect waves-effect waves-ligth deleteRecord">Excluir</button>
                </div>
            </div>
        </div>
    </div>
    ...
    firebase.database().ref('customers/').on('value', function(snapshot){
        ...
        $.each(value, function (index, value) {
            if (value) {
                ...
                <td>' + value.name + '</td>\
                <td>' + value.email + '</td>\
                <td><button data-toggle="modal" data-target="#update-modal" class="btn btn-info updateData" data-id="' + index + '">Alterar</button>\
                <button data-toggle="modal" data-target="#remove-modal" class="btn btn-danger removeData" data-id="' + index + '">Excluir</button></td>\
            </tr>');
            }
            lastIndex = index;
        });
        $('#tbody').html(htmls);
        $("#submitUser").removeClass('disable');
    });

    $('#submitCliente').on('click', function(){
        var values = $("#addCliente").serializeArray();
